@php
    ...
    $entityName = $entityName ?? 'mục';
    if (!function_exists('buildPages')) {
...
<div class="flex items-center justify-between">
    ...
    <div class="text-sm font-medium" aria-live="polite">
        Hiển thị <span class="font-bold">{{ $paginator->firstItem() }}</span> đến <span class="font-bold">{{ $paginator->lastItem() }}</span> trong tổng số <span class="font-bold">{{ $paginator->total() }}</span> {{ $entityName }}
    </div>
    @else
    <div></div>
    @endif
