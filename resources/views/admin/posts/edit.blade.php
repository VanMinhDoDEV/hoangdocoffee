...
<head>
    ...
    <title>{{ __('messages.update_post') }}</title>
...
<body class="text-slate-900 bg-slate-50">
    ...
    <nav class="sticky top-0 z-50 bg-white border-b border-slate-200 px-6 py-3 flex justify-between items-center shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-bold text-lg tracking-tight">{{ __('messages.update_post') }}</h2>
        </div>
        <div class="flex items-center gap-3">
            ...
            <button type="submit" name="action" value="published" class="px-6 py-2 bg-indigo-600 text-white text-sm font-bold rounded-lg shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all">{{ __('messages.update') }}</button>
        </div>
    </nav>
            ...
            <div class="lg:col-span-8 space-y-8">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <span class="text-xs text-slate-400 font-mono" id="title-char-count">{{ mb_strlen($post->title) }}/60 {{ __('messages.characters') }}</span>
                    </div>
                    <input type="text" name="title" id="title" placeholder="{{ __('messages.enter_title') }}" required maxlength="60" value="{{ old('title', $post->title) }}"
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
                    ...
                    <div class="space-y-6">
                        <div class="bg-slate-50 p-6 rounded-xl border border-slate-100 max-w-2xl">
                            ...
                            <div class="text-[#1a0dab] text-xl font-medium mb-1 hover:underline cursor-pointer" id="seo-title-preview">{{ $post->title }}</div>
                            <div class="text-[#4d5156] text-sm line-clamp-2 leading-relaxed" id="seo-desc-preview">{{ $post->excerpt ?? __('messages.seo_desc_preview') }}</div>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                            <div>
                                <label class="block text-sm font-semibold mb-2">{{ __('messages.meta_description') }} <span class="text-red-500">*</span></label>
                                <textarea name="meta_description" id="meta_description" rows="3" placeholder="{{ __('messages.enter_summary') }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none text-sm transition-all">{{ old('meta_description', $post->excerpt) }}</textarea>
                            ...
                            <div>
                                ...
                                <input type="text" name="keywords" value="{{ old('keywords') }}" placeholder="{{ __('messages.keywords_placeholder') }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none text-sm transition-all">
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    ...
                    <div class="aspect-[4/3] bg-slate-50 rounded-xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center relative overflow-hidden group hover:border-indigo-400 hover:bg-indigo-50/30 transition-all cursor-pointer" id="thumbnail-container" onclick="document.getElementById('thumbnail-file').click()">
                        @if($post->thumbnail)
                            <img id="thumbnail-preview" src="{{ asset('storage/' . $post->thumbnail) }}" class="absolute inset-0 w-full h-full object-cover">
                            <i class="fa-solid fa-images text-slate-300 text-4xl mb-3 group-hover:scale-110 transition-transform hidden" id="placeholder-icon"></i>
                        @else
                            <i class="fa-solid fa-images text-slate-300 text-4xl mb-3 group-hover:scale-110 transition-transform" id="placeholder-icon"></i>
                            <p class="text-xs font-semibold text-slate-500" id="placeholder-text">{{ __('messages.upload_or_drag') }}</p>
                            <p class="text-[10px] text-slate-400 mt-1" id="placeholder-sub">{{ __('messages.image_support') }}</p>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-3">
                                <label class="flex items-center gap-3 cursor-pointer group">
                                    <input type="checkbox" name="categories[]" value="{{ $category->id }}" {{ $post->category_id == $category->id ? 'checked' : '' }} class="w-4 h-4 rounded text-indigo-600 border-slate-300 focus:ring-indigo-500">
                                ...
                                <div class="ml-6 space-y-3 border-l-2 border-slate-100 pl-4">
                                    ...
                                    <label class="flex items-center gap-3 cursor-pointer group">
                                        <input type="checkbox" name="categories[]" value="{{ $child->id }}" {{ $post->category_id == $child->id ? 'checked' : '' }} class="w-4 h-4 rounded text-indigo-600 border-slate-300 focus:ring-indigo-500">
                                    ...
                                    <div class="ml-4 space-y-3 border-l-2 border-slate-100 pl-4">
                                        ...
                                        <label class="flex items-center gap-3 cursor-pointer group">
                                            <input type="checkbox" name="categories[]" value="{{ $subChild->id }}" {{ $post->category_id == $subChild->id ? 'checked' : '' }} class="w-3.5 h-3.5 rounded text-indigo-600 border-slate-300 focus:ring-indigo-500">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-4">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-sm font-bold">{{ $post->author->name ?? auth()->user()->name }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        ...
                        @if($post->status == 'published')
                            <span class="bg-green-100 text-green-700 text-[10px] font-extrabold px-2 py-1 rounded">{{ strtoupper(__('messages.published')) }}</span>
                        @else
                            <span class="bg-amber-100 text-amber-700 text-[10px] font-extrabold px-2 py-1 rounded">{{ strtoupper(__('messages.draft')) }}</span>
                        @endif
                    </div>
                    <div class="flex justify-between items-center">
                         <span class="text-sm text-slate-500 font-medium italic"><i class="fa-solid fa-eye mr-2"></i> {{ __('messages.views') }}:</span>
                         <span class="text-sm font-bold">{{ number_format($post->views) }}</span>
                    </div>
                </div>
    ...
    <script>
        ...
        // Initialize tags from server
        let tags = @json($post->tags->pluck('name'));
